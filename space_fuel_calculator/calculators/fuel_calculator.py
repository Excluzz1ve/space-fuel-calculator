...
class FuelCalculator:
    ...
    def __init__(self):
        ...
        # Создаем объект Земли для расчетов
        from ..data.planets import PLANETS_DATA
        self.earth = PLANETS_DATA['earth']
    ...
    def calculate_round_trip_fuel(self, destination: Planet, payload_mass: float, engine: Engine) -> FuelResult:
        """
        Рассчитывает топливо для полета туда и обратно с исправленными значениями дельта-V.
        ...
        # Используем TrajectoryCalculator для правильного расчета дельта-V
        outbound_delta_v, return_delta_v = self.trajectory_calc.calculate_roundtrip_delta_v(
            self.earth, destination
        )
        
        # Общая дельта-V
        total_delta_v = outbound_delta_v + return_delta_v
        
        # Расчет топлива для каждого этапа
        outbound_result = self.calculate_fuel_mass(outbound_delta_v, payload_mass, engine)
        return_result = self.calculate_fuel_mass(return_delta_v, payload_mass, engine)
        
        return FuelResult(
            outbound_fuel=outbound_result.total_fuel,
            return_fuel=return_result.total_fuel,
            total_fuel=outbound_result.total_fuel + return_result.total_fuel,
            delta_v_outbound=outbound_delta_v,
            delta_v_return=return_delta_v,
